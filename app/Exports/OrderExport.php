...
class OrderExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithCustomStartCell, ShouldAutoSize//, WithEvents
{
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $styles["A{$tableHeaderRow}:D{$tableHeaderRow}"] = [
            'font' => [
                'bold' => true,
                'color' => ['argb' => 'FFFFFFFF']
            ],
        ...
        if ($tableDataEndRow >= $tableDataStartRow) {
            $styles["A{$tableDataStartRow}:D{$tableDataEndRow}"] = [
                'borders' => [
                    'allBorders' => [
                        'borderStyle' => Border::BORDER_THIN,
                    ]
            ...
            $styles["C{$tableDataStartRow}:D{$tableDataEndRow}"] = [
                'alignment' => [
        ...
        // Fila del subtotal
        $styles["C{$subTotalRow}:D{$subTotalRow}"] = [
            'font' => [
                // 'bold' => true,
                'size' => 12
            ],
        ...
        // Fila del ajuste
        $styles["C{$adjustmentRow}:D{$adjustmentRow}"] = [
            'font' => [
                // 'bold' => true,
                'size' => 12
            ],
